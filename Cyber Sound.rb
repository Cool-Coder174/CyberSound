...
live_loop :kick do
  if (spread 1, 4).tick then # 4-on-the-floor
    sample :bd_fat, amp: master.look * kick_volume,
...
define :snare do |amp|
  sample :sn_dub, amp: amp, start: 0.15, finish: 0.35, rate: 0.7
end
...
live_loop :hats do
  ...
  if (spread 3, 8).tick then
    ...
    with_fx :rbpf, cutoff: 120, res: 0.8 do
  ...
  mix: 0.4, decay: 4, phase: 0.75 do
    with_fx :hpf, cutoff: open_hats_cutoffs.tick do
      with_fx :reverb, mix: 0.4, room: 0.8 do
        sleep 0.5
        sample :drum_cymbal_closed, start: 0.2, finish: 0.3, amp: 0.5
        sleep 0.5
      end
    end
  end
...
bassline_rhythm = (ring 1, 0, 0.5, 0, 1, 0, 0.5, 0, 1, 0.5, 0.5, 0, 1, 0, 0.5, 0)
bassline_notes = (stretch [:d1] * 12 + [:g1, :f1, :a1, :f1], 8)
live_loop :bassline do
  sync :kick
  with_synth :fm do
    play bassline_notes.look,
      amp: master.look * bassline_rhythm.tick * bass_volume,
      attack: 0.03, divisor: 1, depth: 3
  end
  sleep 0.25
end
...
live_loop :revbassline do
  ...
  with_fx :pan, pan: -0.5 do
    with_synth :fm do
      play :a1, amp: bass_volume * master.tick,
...
dchord = chord(:a2, :m7, num_octaves: 4)
      ...
      with_fx :reverb, room: 0.7, damp: 0.8 do
        with_synth_defaults attack: 0.05, release: 0.3 do
          with_transpose synth_transpositions.look do
            ...
            with_fx :wobble, cutoff_min: cutoff,
...
dubpad_cutoffs = range(69, 100, 5).mirror
dubpad_phases = (ring 8, 16, 8, 0.5)
...
beep_notes = (ring :d2, :a2, :f2, :e2, :a3, :g2)
live_loop :beeps do
  sync :kick
  sleep 0.5
